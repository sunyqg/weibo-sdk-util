...
{
   char buffer[32];
#if defined( __STDC_SECURE_LIB__) && !defined(WINCE) // Use secure version with visual studio 2005 to avoid warning.
...
std::string valueToString( Value::Int64 value)
{
   ...
   if ( isNegative )
      value = -value;
   uint64ToString( Value::UInt64(value), current );
   if ( isNegative )
      *--current = '-';
   assert( current >= buffer );
   return current;
...
std::string valueToString( Value::UInt64 value )
{
   char buffer[64];
   char *current = buffer + sizeof(buffer);
...
int IsGB(char ch)
{
	unsigned char sqChar = (unsigned char)ch;
	if (sqChar>0x80)
	{
		return 1;
	}
